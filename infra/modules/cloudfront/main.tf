...
resource "aws_cloudfront_distribution" "frontend" {
  origin {
    ...
    custom_header {
      name  = "X-Custom-CloudFront-Secret"
      value = var.cloudfront_secret
    }
  }

  origin {
    domain_name = var.agents_alb_dns_name
    origin_id   = "ALB-${var.project_name}-agents"
  ...
  ordered_cache_behavior {
    ...
    allowed_methods        = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods         = ["GET", "HEAD"]
    target_origin_id       = "ALB-${var.project_name}-agents"
    compress               = true
    viewer_protocol_policy = "redirect-to-https"
    forwarded_values {
      query_string = true
  ...
  ordered_cache_behavior {
    path_pattern           = "/organization_profile/*"
    allowed_methods        = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods         = ["GET", "HEAD"]
    target_origin_id       = "ALB-${var.project_name}-agents"
    compress               = true
  ...
  # checkov:skip=CKV_AWS_174: TLS 1.2 is configured via minimum_protocol_version
  viewer_certificate {
...
resource "aws_wafv2_web_acl" "cloudfront_waf" {
  ...
  # AWS Managed Rules - Known Bad Inputs (includes Log4Shell protection)
  rule {
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "CloudFrontKnownBadInputsRuleSet"
      sampled_requests_enabled   = true
    }
  }
