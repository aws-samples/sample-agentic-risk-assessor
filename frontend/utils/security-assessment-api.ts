...
export const getSecurityAssessments = async (projectId: string) => {
  try {
    const response = await api.get(`/api/projects/${projectId}/security-assessments`);
    return { data: response.data, error: null };
  } catch (error: any) {
    console.error('Error getting security assessments:', error);
    return { 
      data: null, 
      error: { 
        message: error.response?.data?.message || 'Failed to get security assessments'
      }
    };
  }
};
